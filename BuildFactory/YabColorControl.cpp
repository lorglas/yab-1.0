...
YabColorControl::YabColorControl(BPoint start,color_control_layout layout,float cell_size,const char *name,BMessage *message,bool use_offscreen)
...
void YabColorControl::MouseDown(BPoint where)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
...
void YabColorControl::MouseUp(BPoint pt)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
	GetMouse(&ptCursor, &uButtons, false); 
...
void YabColorControl::MouseMoved(BPoint pt, uint32 code, const BMessage *msg)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
	...
	BColorControl::MouseMoved(pt, code, msg);
}
