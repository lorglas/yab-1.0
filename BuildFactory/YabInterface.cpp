...
#ifdef BUILD_BUBBLEHELPER
	#include "BubbleHelper.h"
#endif
#ifdef BUILD_CALENDAR
	#include "CalendarControl.h"
#endif
#ifdef BUILD_FILEPANEL
	#include "YabFilePanel.h"
#endif
#ifdef BUILD_SPLITPANE
	#include "SplitPane.h"
#endif
#ifdef BUILD_URLVIEW
	#include "URLView.h"
...
#endif
#include "YabMenu.h"
#ifdef BUILD_YABSTACKVIEW
	#include "YabStackView.h"
#endif
#ifdef BUILD_COLUMNLISTVIEW
			...
			if(child = myView->ChildAt(0))
			{
				while(child)
				{
						...
						if(is_kind_of(child, YabTabView))
						{
							for(int i = 0; i<((YabTabView*)child)->CountTabs(); i++)
							{
								YabView *t = (YabView*)(&(((YabTabView*)child)->TabAt(i)).GetTargetView());
								RemoveView(t);
								viewList->DelView(t->Name());
							}
						}
					...
					if(subchild = child->ChildAt(0))
						while(subchild)
						{
								...
								if(is_kind_of(subchild, YabTabView))
								{
									for(int i = 0; i<((YabTabView*)subchild)->CountTabs(); i++)
									{
										YabView *t = (YabView*)(&(((YabTabView*)subchild)->TabAt(i)).GetTargetView());
										RemoveView(t);
										viewList->DelView(t->Name());
									}
								}
...
void YabInterface::BoxView(BRect frame, const char* id, const char* text, int lineType, const char* view)
{
	...
	if(myView)
	{
		...
		{
			w->Lock();
			BBox *newBox = new BBox(frame, "bbox", B_FOLLOW_ALL_SIDES, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE|B_NAVIGABLE_JUMP);
			YabView *newView = new YabView(BRect(3,12,frame.Width()-3,frame.Height()-3), id, B_FOLLOW_ALL_SIDES, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE|B_NAVIGABLE_JUMP);
			if(w->layout == -1)
			{
...
{
	#ifndef BUILD_TABVIEW 
		ErrorGen("yab was not compiled with this command!");
	#else
			
	BString option(mode);
	#ifdef BUILD_HAIKUTAB
	/*
		if(option.IFindFirst("top")!=B_ERROR) side = TAB_NORTH;
		else if(option.IFindFirst("bottom")!=B_ERROR) side = TAB_SOUTH;
		else if(option.IFindFirst("left")!=B_ERROR) side = TAB_WEST;
		else if(option.IFindFirst("right")!=B_ERROR) side = TAB_EAST;
		...
		*/
		...
		if(option.IFindFirst("top")!=B_ERROR) side = toTop;
		else if(option.IFindFirst("bottom")!=B_ERROR) side = toBottom;
		// else if(option.IFindFirst("left")!=B_ERROR) side = toLeft;
		// else if(option.IFindFirst("right")!=B_ERROR) side = toRight;
		else ErrorGen("Invalid Option");
	#endif
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			#ifdef BUILD_HAIKUTAB
				// YTabView *myTabView = new YTabView(frame, id);
				YabTabView *myTabView = new YabTabView(frame, id);
			#else
				YabTabView *myTabView = new YabTabView(frame, id);
			#endif

			// TabView *myTabView = new TabView(frame, id, newnames, viewnum);
			if(w->layout == -1)
			...
			#else
				myTabView->SetTabOrientation(side);
			#endif
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			if(w)
			{
				w->Lock();
				#ifdef BUILD_HAIKUTAB
					// myTabView = cast_as(myView->FindView(id), YTabView);
				...
				if(myTabView)
				{
					...
					/*
						...
						tab_orientation side = myTabView->GetOrientation();
						switch(side)
						{
							case TAB_NORTH: contentFrame.top -= myTabView->GetHeight();
									break;
							case TAB_EAST: contentFrame.right -= myTabView->GetHeight();
									break;
							case TAB_SOUTH: contentFrame.bottom -= myTabView->GetHeight();
									break;
							case TAB_WEST: contentFrame.left -= myTabView->GetHeight();
									break;
						}
						*/
						BRect contentFrame = myTabView->Bounds();
					#else
						BRect contentFrame = myTabView->GetContentArea();
					#endif
					...
					viewList->AddView(t.String(), newView, TYPE_YABVIEW);
						...
						/*
						int32 n = myTabView->SelectedTab();
						myTabView->AddTab((BView*)newView, tabname);
						myTabView->SelectTab(n);
						*/
					#else
						myTabView->AddTab(new BStringTab(BString(tabname), (BView*)newView));
						myTabView->SelectTab(myTabView->GetSelectedIndex());
					#endif
					w->Unlock();
					return;
				}
						...
						while(child)
						{
							...
							if(subchild = child->ChildAt(0))
								while(subchild)
								{
									...
									#endif
									#ifdef BUILD_ZETATAB
										if(is_kind_of(subchild, BTabView))
										{
											for(int i = 0; i<((BTabView*)subchild)->CountTabs(); i++)
											{
												YabView *t = (YabView*)(&(((BTabView*)subchild)->TabAt(i)).GetTargetView());
												RemoveView(t);
												viewList->DelView(t->Name());
											}
										}
		...
		if(myView)
		{
			...
			if(w)
			{
				...
				if(myTabView)
				{
					...
					#else
						ret = myTabView->GetSelectedIndex();
					#endif
					w->Unlock();
					return ret+1;
				}
...
void YabInterface::CreateButton(BRect frame, const char* id, const char* title, const char* view)
{
	...
	if(myView)
	{
		...
		{
			w->Lock();
			YabButton* myButton = new YabButton(frame,id,title,new BMessage(YABBUTTON));
			if(w->layout == -1)
...
void YabInterface::StatusBar(BRect frame, const char* id, const char* label1, const char* label2, const char* view)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			bar->SetBarHeight((float)frame.Height()-13);
			myView->AddChild(bar);
			bar->Draw(frame);
			w->Unlock();
		}
...
void YabInterface::CreateTextControl(BRect frame, const char* id, const char* label, const char* text, const char* window)
{
	...
	if(myView)
	{
		...
		{ 
			w->Lock();
			YabTextControl *textControl = new YabTextControl(frame, id ,label, text, new BMessage(YABTEXTCONTROL));
...
void YabInterface::CreateCheckBox(double x, double y, const char* id, const char* label, int isActivated, const char* window)
{
	...
	if(myView)
	{
		...
		if(w) 
		{
			...
			YabCheckBox *checkBox = new YabCheckBox(frame, id, label, new BMessage(YABCHECKBOX));
...
void YabInterface::CreateRadioButton(double x, double y, const char* groupID, const char* label, int isActivated, const char* window)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			w->Lock();
			BRect frame(x,y,x+1,y+1);
			YabRadioButton *radio = new YabRadioButton(frame, groupID, label, new BMessage(YABRADIOBUTTON));
...
void YabInterface::CreateListBox(BRect frame, const char* title, int scrollbar, const char* window)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			YabListView  *list = new YabListView(frame,title);
			if(w->layout == -1)
			...
			{
				case 3:  // both
					myView->AddChild(new YabScrollView("scroll_list", list, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, true, true));
					break;
				case 2:  // horizontal
					myView->AddChild(new YabScrollView("scroll_list", list, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, true, false));
					break;
				case 0:  // none
					myView->AddChild(list);
					break;
				default: // vertical is default
					myView->AddChild(new YabScrollView("scroll_list", list, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, false, true));
					break;
			}
			w->Unlock();
		}
...
{ 
	...
	if(myView)
	{
		...
		{
			w->Lock();
			YabPopUpMenu *dropmenu = new YabPopUpMenu("");
			#ifdef ZETA
				YabMenuField *drop = new YabMenuField(frame,title,label, dropmenu, true);
			#else
				YabMenuField *drop = new YabMenuField(frame,title,label, dropmenu);
			#endif
...
void YabInterface::CreateItem(const char* id, const char* item)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				YabMenuField *myMenuField = cast_as(myView->FindView(id), YabMenuField);
				if(myMenuField)
				{
					YabPopUpMenu *myPopup = cast_as(myMenuField->Menu(), YabPopUpMenu);
					if(myPopup) 
					{
						if(!strcmp(item,"--"))
...
void YabInterface::RemoveItem(const char* title, const char* item)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				YabListView *myList = cast_as(myView->FindView(title), YabListView);
				if(myList) 
				{
...
void YabInterface::CreateText(double x, double y, const char* id, const char* text, const char* window)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			YabStringView *s = new YabStringView(BRect(x,y-3,x+b,y+h-3), id, text);
			s->ResizeToPreferred();
			if(w->layout == -1)
...
void YabInterface::Text2(BRect frame, const char* id, const char* text, const char* window)
{
	...
	if(myView)
	{
		...
		{
			w->Lock();
			YabStringView *s = new YabStringView(frame, id, text);
			if(w->layout == -1)
				s->SetResizingMode(B_FOLLOW_RIGHT|B_FOLLOW_BOTTOM);
			else
				s->SetResizingMode(w->layout);
...
void YabInterface::TextAlign(const char* txt, const char* option)
{
	...
	YabView *myView = NULL;
	YabStringView *myStringView = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myStringView = cast_as(myView->FindView(txt), YabStringView);
				if(myStringView)
				{
...
void YabInterface::Slider(BRect frame, const char* id, const char* title, int min, int max, const char* view)
{
	...
	if(myView)
	{
		...
		{
			w->Lock();
			YabSlider *mySlider = new YabSlider(frame, id, title, new BMessage(YABSLIDER), min, max);
			if(w->layout == -1)
...
void YabInterface::SetSlider(const char* id, const char* label1, const char* label2)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				mySlider = cast_as(myView->FindView(id), YabSlider);
				if(mySlider)
				{
			...
			if(w)
			{
				...
				if(target)
				{
					if(isEnabled)
					{
						...
						{
							YabMenuField *myMenu = cast_as(target, YabMenuField);
							if(myMenu)
...
void YabInterface::ColorControl(double x, double y, const char* id, const char* view)
{
	...
	if(myView)
	{
		...
		{
			w->Lock();
			YabColorControl *myCControl = new YabColorControl(BPoint(x,y), B_CELLS_32x8, 2, id, new BMessage(YABCOLORCONTROL),false);
			if(w->layout == -1)
...
{
	YabView *myView = NULL;
	YabColorControl *myCControl = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myCControl = cast_as(myView->FindView(id), YabColorControl);
				if(myCControl)
				{
...
{
	YabView *myView = NULL;
	YabTextControl *myTControl = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myTControl = cast_as(myView->FindView(id), YabTextControl);
				if(myTControl)
				{
...
void YabInterface::TextEdit(BRect frame, const char* title, int scrollbar, const char* window)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			{
				case 3:  // both
					myView->AddChild(new YabScrollView("scroll_list", txtView, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, true, true));
					break;
				case 2:  // horizontal
					myView->AddChild(new YabScrollView("scroll_list", txtView, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, true, false));
					break;
				case 0:  // none
					myView->AddChild(txtView);
					break;
				default: // vertical is default
					myView->AddChild(new YabScrollView("scroll_list", txtView, resizeMode, B_WILL_DRAW|B_FULL_UPDATE_ON_RESIZE, false, true));
					break;
			}
			w->Unlock();
		}
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			if(w)
			{
				...
				{
				/*
					if(tmp.IFindFirst("alignment")!=B_ERROR)
					{
						if(tmp2.IFindFirst("left"))
							myText->SetAlignment(B_ALIGN_LEFT);
						else if(tmp2.IFindFirst("center"))
							myText->SetAlignment(B_ALIGN_CENTER);
						else if(tmp2.IFindFirst("right"))
							myText->SetAlignment(B_ALIGN_RIGHT);
						else
							ErrorGen("Unknown option");
					}
					else
					if(tmp.IFindFirst("font")!=B_ERROR)
						; */
					if(tmp.IFindFirst("autocomplete")!=B_ERROR)
...
void YabInterface::TreeBox1(BRect frame, const char* id, int scrollbarType, const char* view)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			YabOutlineListView  *list = new YabOutlineListView(frame,id);
			if(w->layout == -1)
				resizeMode = B_FOLLOW_ALL;
			else
				resizeMode = w->layout;
...
{
	YabView *myView = NULL;
	YabOutlineListView *myTree = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myTree = cast_as(myView->FindView(id), YabOutlineListView);
				if(myTree)
				{
...
const char* YabInterface::TreeboxGet(const char* treebox, int pos)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				myTree = cast_as(myView->FindView(treebox), YabOutlineListView);
				if(myTree)
				{
...
void YabInterface::ButtonImage(double x,double y, const char* id,const char* enabledon, const char* enabledoff, const char* disabled, const char* view)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			YabPictureButton *myButton = new YabPictureButton(r, id, pic2, pic1, new BMessage(YABBUTTON));
...
void YabInterface::CheckboxImage(double x, double y,const char* id,const char* enabledon, const char* enabledoff, const char *disabledon, const char *disabledoff, int isActivated, const char* view)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			...
			YabPictureButton *myButton = new YabPictureButton(r, id, pic2, pic1, new BMessage(YABCHECKBOX),B_TWO_STATE_BUTTON);
...
{
	YabView *myView = NULL;
	YabCheckBox *myCheckBox = NULL;
	YabPictureButton *myPicButton = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myCheckBox = cast_as(myView->FindView(id), YabCheckBox);
				if(myCheckBox)
				{
				...
				{
					myPicButton = cast_as(myView->FindView(id), YabPictureButton);
					if(myPicButton)
					{
...
{
	YabView *myView = NULL;
	YabRadioButton *myRadioButton= NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myRadioButton = cast_as(myView->FindView(id), YabRadioButton);
				if(myRadioButton)
				{
					...
					if(theView->Name())
					{
						if(!strcmp(theView->Name(), view))
						{
							if(text[0] == '\0')
									...
									#endif
								#else
									theView->SetToolTipText("");
								#endif
							else
								#ifndef ZETA
									#ifdef BUILD_BUBBLEHELPER
										tooltip->SetHelp(theView, text);
									#else
										;
									#endif
								#else
									theView->SetToolTipText(text);
								#endif
							w->Unlock();
							return;
						}
...
void YabInterface::TreeSort(const char* view)
{
	...
	YabView *myView = NULL;
	YabOutlineListView *myList = NULL;
	for(int i=0; i<viewList->CountItems(); i++)
	{
		myView = cast_as((BView*)viewList->ItemAt(i), YabView);
		if(myView)
		{
			...
			{
				w->Lock();
				myList = cast_as(myView->FindView(view), YabOutlineListView);
				if(myList)
				{
...
void YabInterface::ListSort(const char* view)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				myList = cast_as(myView->FindView(view), YabListView);
				if(myList)
				{
...
void YabInterface::ListboxAdd(const char* listbox, const char* item)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				myList = cast_as(myView->FindView(listbox), YabListView);
			...
			if(myView)
			{
				...
				if(w)
				{
					...
					if(myBView)
					{
						...
						{
							YabScrollView *myScrollView = new YabScrollView(id, myBView, B_FOLLOW_LEFT|B_FOLLOW_TOP, 0, hasHor, hasVer);
							if(w->layout == -1)
...
int YabInterface::ColorControlGet(const char* colorcontrol, const char* option)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView && myOption>-1)
		{
			...
			{
				w->Lock();
				myCControl = cast_as(myView->FindView(colorcontrol), YabColorControl);
				if(myCControl)
				{
...
int YabInterface::SliderGet(const char* slider)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				mySlider = (YabSlider*)myView->FindView(slider);
				if(mySlider)
				{
					ret = mySlider->Value();
					w->Unlock();
...
const char* YabInterface::PopUpMenu(double x, double y, const char* menuItems, const char* view)
{
	...
	if(myView)
	{
		...
		if(w)
		{
			const char* res;
			w->Lock();
			YabPopUpMenu *returnMe = new YabPopUpMenu( "YabPopup", false, false );
...
void YabInterface::DropBoxSelect(const char* dropbox, int position)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				YabMenuField *myMenuField = cast_as(myView->FindView(dropbox), YabMenuField);
				if(myMenuField)
				{
...
double YabInterface::TabHeight()
{
	...
	f.GetHeight(&fh);
			...
			return fh.ascent + fh.descent + fh.leading + 15.0f; 
		#else
...
const char* GetMMsgInfo(int mouseStateInfo, int mouseLButton, int mouseMButton, int mouseRButton, const char* name)
{
	char _tmp[500];
	_tmp[0]='\0';
	switch(mouseStateInfo)
	{
		case 0:
			strcat(_tmp,name);
			strcat(_tmp,":_InsideView");
 			strcat(_tmp,"|");
			break;
		case 1:
			strcat(_tmp,name);
			strcat(_tmp,":_EnteredView");
 			strcat(_tmp,"|");
			break;
		case 2:
			//tmp = name;
		...
		case 3:
			strcat(_tmp,name);
			strcat(_tmp,":_ExitedView");
 			strcat(_tmp,"|");
			break;
		case 4:
			strcat(_tmp,name);
			strcat(_tmp,":_MouseDown");
 			strcat(_tmp,"|");
			break;
		case 5:
			strcat(_tmp,name);
			strcat(_tmp,":_MouseUp");
 			strcat(_tmp,"|");
			break;
	}
	if (mouseLButton==1)
		{
			strcat(_tmp,name);
			strcat(_tmp,":_LeftMouseButton");
 			strcat(_tmp,"|");
 			//myView->mouseLButton=0;
		}
	if (mouseRButton==1)
		{
			strcat(_tmp,name);
			strcat(_tmp,":_RightMouseButton");
 			strcat(_tmp,"|");
 			//myView->mouseRButton=0;
		}
	if (mouseMButton==1)
		{
			strcat(_tmp,name);
			strcat(_tmp,":_MiddleMouseButton");
 			strcat(_tmp,"|");
 			//myView->mouseMButton=0;
		}	
	return _tmp;	
}
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{			
				...
				strcat(thisTmp,GetMMsgInfo(myView->mouseStateInfo, myView->mouseLButton,myView->mouseMButton,myView->mouseRButton,myView->Name()));
				myView->mouseLButton=0;
				myView->mouseRButton=0;
				myView->mouseMButton=0;
				myView->mouseStateInfo=-1;
				...
				if (myView->CountChildren())
				{
					...
					{
						handled=-1;
  					    if (dynamic_cast<YabButton*>(myView->ChildAt(o)))
  					    {
							YabButton *target = dynamic_cast<YabButton*>(myView->ChildAt(o));
							if (target)
							{	
								handled = 1;
								
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
								target->mouseLButton=0;
								target->mouseRButton=0;
								target->mouseMButton=0;
								target->mouseStateInfo=-1;
							}
  					    }
					
					    #ifdef BUILD_COLUMNLISTVIEW
  					    if (dynamic_cast<BColumnListView*>(myView->ChildAt(o)))
  					    {
							BColumnListView *target = dynamic_cast<BColumnListView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
					    ...
					    #endif
  					    if (dynamic_cast<YabPictureButton*>(myView->ChildAt(o)))
  					    {
							YabPictureButton *target = dynamic_cast<YabPictureButton*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabTextControl*>(myView->ChildAt(o)))
  					    {
							YabTextControl *target = dynamic_cast<YabTextControl*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
						...
						if (dynamic_cast<YabOutlineListView*>(myView->ChildAt(o)))
  					    {
							YabOutlineListView *target = dynamic_cast<YabOutlineListView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
					    ...
					    #ifdef BUILD_YABTEXT
  					    if (dynamic_cast<YabText*>(myView->ChildAt(o)))
  					    {
							YabText *target = dynamic_cast<YabText*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
					    ...
					    #endif
  					    if (dynamic_cast<YabListView*>(myView->ChildAt(o)))
  					    {
							YabListView *target = dynamic_cast<YabListView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabScrollView*>(myView->ChildAt(o)))
  					    {
							YabScrollView *target = dynamic_cast<YabScrollView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
								...
								for (int o=0; o<target->CountChildren(); o++)
								{

					    			#ifdef BUILD_YABTEXT
  					    			if (dynamic_cast<YabText*>(target->ChildAt(o)))
  					    			{
										YabText *target2 = dynamic_cast<YabText*>(target->ChildAt(o));
										if (target2)
										{
											handled = 1;
											strcat(thisTmp,GetMMsgInfo(target2->mouseStateInfo, target2->mouseLButton,target2->mouseMButton,target2->mouseRButton,target2->Name()));
											target2->mouseLButton=0;
											target2->mouseRButton=0;
											target2->mouseMButton=0;
											target2->mouseStateInfo=-1;
										}
			  					    }									
								    #endif
									if (dynamic_cast<YabOutlineListView*>(target->ChildAt(o)))
  					    			{
										YabOutlineListView *target2 = dynamic_cast<YabOutlineListView*>(target->ChildAt(o));
										if (target2)
										{
											handled = 1;
											strcat(thisTmp,GetMMsgInfo(target2->mouseStateInfo, target2->mouseLButton,target2->mouseMButton,target2->mouseRButton,target2->Name()));
									...
									if (dynamic_cast<YabListView*>(target->ChildAt(o)))
  					    			{
										YabListView *target2 = dynamic_cast<YabListView*>(target->ChildAt(o));
										if (target2)
										{
											handled = 1;
											strcat(thisTmp,GetMMsgInfo(target2->mouseStateInfo, target2->mouseLButton,target2->mouseMButton,target2->mouseRButton,target2->Name()));
  					    ...
  					    if (dynamic_cast<YabCheckBox*>(myView->ChildAt(o)))
  					    {
							YabCheckBox *target = dynamic_cast<YabCheckBox*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabMenuField*>(myView->ChildAt(o)))
  					    {
							YabMenuField *target = dynamic_cast<YabMenuField*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabBitmapView*>(myView->ChildAt(o)))
  					    {
							YabBitmapView *target = dynamic_cast<YabBitmapView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabPopUpMenu*>(myView->ChildAt(o)))
  					    {
							YabPopUpMenu *target = dynamic_cast<YabPopUpMenu*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabRadioButton*>(myView->ChildAt(o)))
  					    {
							YabRadioButton *target = dynamic_cast<YabRadioButton*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabSlider*>(myView->ChildAt(o)))
  					    {
							YabSlider *target = dynamic_cast<YabSlider*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabColorControl*>(myView->ChildAt(o)))
  					    {
							YabColorControl *target = dynamic_cast<YabColorControl*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
  					    ...
  					    if (dynamic_cast<YabStringView*>(myView->ChildAt(o)))
  					    {
							YabStringView *target = dynamic_cast<YabStringView*>(myView->ChildAt(o));
							if (target)
							{
								handled = 1;
								strcat(thisTmp,GetMMsgInfo(target->mouseStateInfo, target->mouseLButton,target->mouseMButton,target->mouseRButton,target->Name()));
						...
						if (handled!=1)
						{
							//fprintf(stderr,"Control not handled: %s",myView->ChildAt(o)->Name());
						}
...
void YabInterface::BitmapGet(double w, const char* id, const char* path)
{
	...
	BEntry fEntry = BEntry( path );
	BBitmap icon = &GetTrackerIcon( fEntry, w );

	fBitmap->Lock();
	bview->DrawBitmap( &icon, iFrame );
	fBitmap->Unlock();
...
void YabInterface::BitmapGetIcon(const char* id, const char* option, const char* path)
{
	...
	else if( fString.IFindFirst("Mime") != B_ERROR )
	{
		...
		BBitmap icon = &GetTrackerIcon(path, w);
		fBitmap->Lock();
		bview->DrawBitmap( &icon, iFrame );
		bview->Sync();
		fBitmap->Unlock();
...
void YabInterface::DrawSet(const char* option, const char* color,const char* view)
{
	...
	{
		#ifdef ZETA
			col = ui_color(B_PANEL_LINK_COLOR);
		#else
...
int YabInterface::ListboxGetNum(const char* id)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				myList = cast_as(myView->FindView(id), YabListView);
				if(myList)
				{
...
int YabInterface::TreeboxGetNum(const char* id)
{
	...
	for(int i=0; i<viewList->CountItems(); i++)
	{
		...
		if(myView)
		{
			...
			{
				w->Lock();
				myList = cast_as(myView->FindView(id), YabOutlineListView);
				if(myList)
				{
...
{
	#ifdef ZETA
		be_locale.LoadLanguageFile(path);
	#endif
}
