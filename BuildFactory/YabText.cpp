...
{
	void *anItem;
	for ( int32 i = 0; (anItem = words->ItemAt(i)); i++ )
		delete anItem;
	delete words; 
}
...
void YabText::MouseDown(BPoint where)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
...
void YabText::MouseUp(BPoint where)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
	GetMouse(&ptCursor, &uButtons, false); 
...
void YabText::MouseMoved(BPoint where, uint32 code, const BMessage *message)
{
	BPoint ptCursor;
	uint32 uButtons = 0;
...
void YabText::FillSolEol(vector<int>& s,vector<int>& e,int start,int finish)
{
	int i=start;
	int text_length = TextLength();
	for(i=start;i>=0;i--)
...
void YabText::ParseAll(int start,int finish,bool IsInteractive)
{
	...
	/*	
		...
		{
			size = text_length;
			vector<rgb_color> colorVec(size,textcolor);
...
void YabText::ParseLine(int sol,int eol,vector<rgb_color>& colorVec)//,vector<rgb_color>& colorVec)
{
	int i;
	int offset = sol;
	int pos;
...
void YabText::ICheckWordLists(int sol,int eol,vector<rgb_color>& colorVec)
{
	int i;
	for(i=sol;i<eol;i++)
	{
...
void YabText::SetText(const char* text,int32 length,const text_run_array* runs = NULL)
{
	hasChanged = true;
...
void YabText::SetText(BFile* file,int32 offset,int32 length,const text_run_array* runs = NULL)
{
	hasChanged = true;
...
bool YabText::Contains(vector<BString>& v,BString s)
{
